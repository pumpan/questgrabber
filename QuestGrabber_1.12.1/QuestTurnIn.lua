...
function menuHandler(available, active, name, complete)
    ...
    if IsShiftKeyDown() then
        ...
        for k = 1, GetNumQuestLogEntries() do
            ...
            -- Have to check if objective is done for bgs becouse they are not marked as completed
            if string.find(title, "Daily: Victory in Warsong Gulch") or string.find(title, "Daily: Victory in ZF Brawl") or string.find(title, "Daily: Victory in Arathi Basin") or string.find(title, "Daily: Victory in Alterac Valley") then
                ...
                -- If any objective is done, mark the quest as done
                if done then
                    --print("Done: " .. title)
                    logCompleted[title] = true
                end
        ...
        -- Check completed quests against the active quests and proceed accordingly
        ...
        -- If no active quests are completed, select and complete available quests
        ...
        -- If no available quests are present, select and complete the first active quest
        if next(active) then
